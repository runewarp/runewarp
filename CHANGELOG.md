...
- Added the role-neutral Managed-session Control downlink: mutually authenticated HTTP/2, exact role-specific SSE paths, snapshot envelope validation, 60-second first-snapshot and silence windows, and full-jitter reconnect that replaces the whole connection on failure. (#TBD)
