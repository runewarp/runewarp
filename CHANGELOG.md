...
- Added managed Server revocation and drain enforcement: Client-identity and Public-hostname authorization changes apply immediately (including during bounded graceful drain), surviving work is remapped by identity continuity, the Managed session stays active through drain until final process exit, and unrecoverable post-commit readiness failures drop readiness and exit nonzero. (#PR)
