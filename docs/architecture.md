...
The client validates the server certificate either through system trust or through `client.server-trust = "ca-file"` with an exclusive CA bundle. The server authenticates one of the Tunnel's pinned `client-identity` values from the client public key rather than the certificate lifetime. Static Server startup loads one **Authorization snapshot** shared by Public-hostname routing and the live QUIC Client-identity verifier; a prepared replacement can be validated beside the live snapshot and committed atomically without exposing a mixed view. Static Client startup seeds one **Address controller** from the configured **Server addresses**, retaining one independent worker per normalized address while preserving today's reconnect, traffic, one-shot Client-ready, and shutdown behavior. In **Managed mode**, each runtime also maintains one role-neutral **Managed session** over mutually authenticated HTTP/2: one role-specific SSE downlink delivers versioned full-input snapshots, a small role-adapter seam applies validated role input, and concurrent state writes on the same connection report only the last successfully applied opaque revision. The managed Server adapter prepares and commits a complete **Authorization snapshot** atomically (including local revocation dispatch and identity-continuous Tunnel-connection remapping), keeps **Server readiness** deferred until the first successful apply, retains the last applied authorization through later Control loss while reconnecting, and keeps the Managed session active through bounded graceful drain so Authorization changes still apply before final process exit. The managed Client adapter atomically replaces **Address controller** maintenance intent from complete `server_addresses` snapshots (including a valid empty assignment), starts independent connect/reconnect workers without awaiting network convergence before acknowledging the revision, leaves removed live connections **Retiring** for remote Server closure (re-adopting without duplicate dialing when re-added before closure), excludes Retiring connections from aggregate assignment convergence, retains the last applied assignment through Control loss while reconnecting, restores no managed input across process restart, and emits convergence transitions plus Retiring events instead of the static one-shot Client-ready event.
...
- managed Server and Client runtimes maintain one **Managed session** Control downlink with mandatory HTTP/2, exactly one role-specific SSE stream, and full-jitter reconnect capped at 60 seconds
- a managed Server binds local listeners with empty authorization, stays Unready until the first successful Server input apply, then atomically replaces Public-hostname routing and Client-identity admission from complete snapshots (including a valid empty Tunnel collection)
- managed Server authorization commits reevaluate live work from Client identity and Public hostname facts: removed identities are denied and disconnected immediately, hostname-only removals reset only affected Visitor streams, and surviving Tunnel connections are remapped when Tunnel ordinals change
- managed Server graceful shutdown loses readiness immediately but keeps the Managed session active through bounded drain so Authorization changes still apply before the HTTP/2 connection closes at final process exit
- managed Server reconciliation emits Received, Applying, Applied, Rejected, and Superseded runtime events without a status endpoint
- a managed Client maintains no Server connections until the first successful Client input apply, then atomically replaces Address-controller maintenance intent from complete Server-address snapshots (including a valid empty assignment) and acknowledges the revision after intent dispatch without awaiting DNS, handshake, or connection success
- managed Client address workers stay independent: one unavailable address never blocks healthy assigned Servers, and removing an Establishing or Reconnecting address cancels unresolved dial/backoff work immediately
- removing a connected Server address marks its worker Retiring without local closure or Infrastructure drain; remote Server shutdown owns Tunnel-connection closure, and re-adding before remote closure re-adopts the live connection without duplicate dialing
- after the first successful Client apply, Control loss retains the last assignment and independent reconnect loops while the session reconnects; process restart restores no managed input, revision, or previously-managed marker and dials nothing until a fresh snapshot applies
- managed Client assignment convergence is separate from applied revision: Converged when every assigned address is connected or the assignment is empty, Partially converged when some are connected, and Unconverged when none of a non-empty assignment are connected; Retiring connections are excluded
- managed Client mode emits per-address Establishing, Connected, Reconnecting, Retiring, Closed, and failed-attempt events plus aggregate convergence transitions, and does not emit the static one-shot Client-ready event
- DNS, connection, authorization, and post-start material failures stay isolated to the affected address worker and retry in-process; unrecoverable controller or worker-task failures exit nonzero
