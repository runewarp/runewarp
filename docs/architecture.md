...
- pending Server `open_bi()` opens are bounded to 1,024 with a 5-second open deadline; each active routed Visitor stream is represented by one registry-owned lease that holds admission and Tunnel-member load accounting, stable routing identity, selective-revocation registration, and exactly-once cleanup; the lease registry also provides event-driven graceful quiescence
