...
Pending stream-open capacity is acquired immediately before `open_bi()` and released on open success, open failure, the **5-second** open deadline, active-stream rejection, cancellation, and shutdown. After a successful open, one synchronously registered active-Visitor-work lease owns active routed-stream capacity, Tunnel-pool member load, routing identity, revocation, and cleanup for the proxy lifetime. Normal return, error, revocation, cancellation, panic, and unpolled drop all release that accounting exactly once. Graceful shutdown waits for the lease registry to become quiescent through an event notification, not periodic polling. Established proxies have no additional lifetime or idle deadline in this policy.
