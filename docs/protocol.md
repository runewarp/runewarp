...
The buffered ClientHello must never be logged or echoed back in diagnostics. With top-level `log-level = "debug"`, stderr diagnostics may log the normalized **Public hostname**, the routing outcome, on the Client the selected `backend-address` or backend failure for the matched **Service**, and separate full-detail lines for runtime tunnel failure causes whose operator-facing `warn` lines are shortened.
