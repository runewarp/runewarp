...
| Caches | Rust and Docker caches are split between PR CI, trusted branch CI, and trusted release scopes. Trusted `main` CI warms the shared trusted-`main` Docker cache that `Images` reuses for publish-time builds, while release rehearsal warms only the separate release Rust cache for the selected tag. Release publish still promotes trusted mainline images instead of consuming PR artifacts. | Cache poisoning and artifact handoff do not bridge the untrusted-to-trusted boundary. |
