...
class ReleaseGatesTest < Minitest::Test
  ...
  def test_rehearsal_mode_accepts_a_matching_release_tag_without_a_git_tag_object
    Dir.mktmpdir do |repo_root|
      ...
      init_git_repo_with_origin_main(repo_root)
      result = run_command(ruby_script("scripts", "validate-release-gates"), "rehearsal", "--repo-root", repo_root, "--tag", "v0.1.0")
  ...
  def test_tag_mode_accepts_an_ssh_signed_tag_from_the_allowed_signers_file
    Dir.mktmpdir do |repo_root|
      ...
      result = run_command(ruby_script("scripts", "validate-release-gates"), "tag", "--repo-root", repo_root, "--tag", "v0.1.0", "--allowed-signers-file", allowed_signers)
      assert(result.success?, result.stderr)
    end
  end
  ...
  def test_tag_mode_can_validate_tagged_release_metadata_from_a_separate_source_tree
    Dir.mktmpdir do |repo_root|
      ...
      result = run_command(ruby_script("scripts", "validate-release-gates"), "tag", "--repo-root", repo_root, "--tag", "v0.1.0", "--allowed-signers-file", allowed_signers, "--metadata-repo-root", release_source)
      assert(result.success?, result.stderr)
    end
  end
