...
class InstallSurfacesTest < Minitest::Test
  ...
  def test_cargo_install_mode_installs_the_binary_and_checks_its_version
    Dir.mktmpdir do |repo_root|
      ...
      result = run_command("ruby", ruby_script("scripts", "validate-install-surfaces.rb"), "cargo-install", "--repo-root", repo_root, "--bin-name", "install-surface-fixture", "--expected-version", "0.3.1")
  ...
  def test_package_readiness_mode_uses_the_sparse_crates_io_protocol
    Dir.mktmpdir do |repo_root|
      ...
      result = run_command("ruby", ruby_script("scripts", "validate-install-surfaces.rb"), "package-readiness", "--repo-root", repo_root, env: { "PATH" => "#{fake_bin_dir}:#{ENV.fetch('PATH')}" })
      assert(result.success?, result.stderr)
  ...
  def test_registry_install_mode_retries_until_the_registry_surface_is_available
    Dir.mktmpdir do |repo_root|
      ...
      result = run_command("ruby", ruby_script("scripts", "validate-install-surfaces.rb"), "registry-install", "--crate-name", "install-surface-fixture", "--bin-name", "install-surface-fixture", "--expected-version", "0.3.1", "--retry-attempts", "3", "--retry-delay-seconds", "0", env: { "PATH" => "#{fake_bin_dir}:#{ENV.fetch('PATH')}" })
      assert(result.success?, result.stderr)
  ...
  def test_registry_install_mode_rejects_zero_retry_attempts
    Dir.mktmpdir do |repo_root|
      write_minimal_binary_crate(repo_root, version: "0.3.1")
      result = run_command("ruby", ruby_script("scripts", "validate-install-surfaces.rb"), "registry-install", "--crate-name", "install-surface-fixture", "--bin-name", "install-surface-fixture", "--expected-version", "0.3.1", "--retry-attempts", "0", "--retry-delay-seconds", "0")
  ...
  def test_docker_registry_image_mode_pulls_and_runs_the_released_image
    Dir.mktmpdir do |repo_root|
      ...
      result = run_command("ruby", ruby_script("scripts", "validate-install-surfaces.rb"), "docker-registry-image", "--image-ref", "docker.io/runewarp/runewarp:0.1.0", "--expected-text", "Usage: runewarp", "--probe-arg", "--help", env: { "PATH" => "#{fake_bin_dir}:#{ENV.fetch('PATH')}" })
      assert(result.success?, result.stderr)
  ...
  def test_docker_registry_image_mode_retries_until_the_image_is_available
    Dir.mktmpdir do |repo_root|
      ...
      result = run_command("ruby", ruby_script("scripts", "validate-install-surfaces.rb"), "docker-registry-image", "--image-ref", "docker.io/runewarp/runewarp:0.1.0", "--expected-version", "0.1.0", "--retry-attempts", "3", "--retry-delay-seconds", "0", env: { "PATH" => "#{fake_bin_dir}:#{ENV.fetch('PATH')}" })
      assert(result.success?, result.stderr)
      assert_equal("3", File.read(attempts_file, encoding: "utf-8"))
    end
  end
  ...
  def test_docker_registry_image_mode_rejects_zero_retry_attempts
    Dir.mktmpdir do |repo_root|
      write_minimal_binary_crate(repo_root, version: "0.3.1")
      result = run_command("ruby", ruby_script("scripts", "validate-install-surfaces.rb"), "docker-registry-image", "--image-ref", "docker.io/runewarp/runewarp:0.1.0", "--expected-version", "0.1.0", "--retry-attempts", "0", "--retry-delay-seconds", "0")
      refute(result.success?)
      assert_includes(result.stderr, "--retry-attempts must be at least 1")
    end
  end
  ...
  def test_docker_registry_tag_absent_mode_allows_a_new_version_tag
    Dir.mktmpdir do |repo_root|
      write_minimal_binary_crate(repo_root, version: "0.3.1")
      result = run_command("ruby", ruby_script("scripts", "validate-install-surfaces.rb"), "docker-registry-tag-absent", "--image-ref", "docker.io/runewarp/runewarp:0.1.0", env: { "RUNEWARP_DOCKER_HUB_STATUS_OVERRIDE" => "404" })
      assert(result.success?, result.stderr)
    end
  end
  ...
  def test_docker_registry_tag_absent_mode_rejects_an_existing_version_tag
    Dir.mktmpdir do |repo_root|
      write_minimal_binary_crate(repo_root, version: "0.3.1")
      result = run_command("ruby", ruby_script("scripts", "validate-install-surfaces.rb"), "docker-registry-tag-absent", "--image-ref", "docker.io/runewarp/runewarp:0.1.0", env: { "RUNEWARP_DOCKER_HUB_STATUS_OVERRIDE" => "200" })
      refute(result.success?)
  ...
  def test_package_readiness_mode_accepts_a_publishable_crate
    Dir.mktmpdir do |repo_root|
      write_minimal_binary_crate(repo_root, version: "0.3.1")
      result = run_command("ruby", ruby_script("scripts", "validate-install-surfaces.rb"), "package-readiness", "--repo-root", repo_root)
      assert(result.success?, result.stderr)
    end
  end
end
